...
// 🔐 API Key และ URL ของ Supabase
const SUPABASE_URL = 'https://tvqfqrreqpuqmytvoajb.supabase.co';
const SUPABASE_KEY = 'EyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InR2cWZxcnJlcXB1cW15dHZvYWpiIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODk5NzIzMDEsImV4cCI6MjEwNTU0ODMwMX0.sM2vfoyHY-BAU2PIhXnJ9NR83D-j6efgK2YjJHmUDqE';
...
// หน้าแรกป้องกัน Server ขึ้น Error
app.get('/', (req, res) => {
    res.send('GG Game Store Backend is Running!');
});

// 1. ดึงข้อมูลผู้ใช้ (User ID: 1)
...
app.post('/api/deposit', async (req, res) => {
    ...
    if (txError) return res.status(500).json({ success: false, message: 'บันทึกสลิปไม่สำเร็จ' });
...
app.post('/api/withdraw', async (req, res) => {
    ...
    if (user.balance < amount) {
        return res.status(400).json({ success: false, message: 'ยอดเงินคงเหลือไม่พอ' });
    }
...
// 6. Admin อนุมัติ/ปฏิเสธ คำขอถอนเงิน
app.post('/api/admin/process-withdraw', async (req, res) => {
    ...
    if (action === 'approve') {
        ...
        res.json({ success: true, message: 'ปฏิเสธคำขอและคืนเงินเรียบร้อย' });
    }
});
